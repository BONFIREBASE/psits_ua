...
export default function AuditManagementPage() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    setFilePreview(null)
    setShowForm(false)
    console.log('[Management] Audit report created:', report)
    toast('Audit report uploaded successfully. (Placeholder)')
  }
    ...
    <div className="p-6 sm:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-white/35 mt-1">
            Upload and manage audit reports and financial audits.
          </p>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        <div className="border border-gold/20 rounded-xl bg-gold/[0.03] p-5 sm:p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-display font-bold text-white">New Audit Report</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <FormField label="Report Title" htmlFor="audit-title" required>
                <input id="audit-title" type="text" value={form.title} onChange={(e) => update('title', e.target.value)} placeholder="e.g. Q1 Financial Audit 2026" className={inputStyles} required />
              </FormField>
              <FormField label="Period Covered" htmlFor="audit-period" required>
                <input id="audit-period" type="text" value={form.period} onChange={(e) => update('period', e.target.value)} placeholder="e.g. January - March 2026" className={inputStyles} required />
              </FormField>
            </div>

            <FormField label="Description / Summary" htmlFor="audit-desc">
              <textarea id="audit-desc" value={form.description} onChange={(e) => update('description', e.target.value)} placeholder="Brief summary of the audit findings..." className={textareaStyles} rows={3} />
            </FormField>

            <FormField label="Attachment (PDF / Image)">
              <FileUpload accept="image/*,.pdf" label="Upload report file" value={file} preview={filePreview} onChange={handleFile} maxSizeMB={10} />
            </FormField>
      ...
      {reports.length === 0 && !showForm ? (
        <EmptyState
          ...
          description="Upload your first audit report to get started. Reports will appear here for review and management."
          action={
            <button
              onClick={() => setShowForm(true)}
